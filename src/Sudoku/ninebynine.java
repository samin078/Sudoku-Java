...
public class ninebynine  implements ActionListener,MouseListener{
    int number = 9;
    int chkcount=3;
    static int chkhint;
    static int k;
    ...
    public ninebynine(int hint, int unknown, int level) {
        chkhint=hint;
        k = unknown;
        lev = level;
    ...
    private void save(){
        ...
        try {
            ...
            Logger.getLogger(ninebynine.class.getName()).log(Level.SEVERE, null, ex);
        }
        }else{
    ...
    private void savePrev(){
        ...
        try {
            ...
            st.executeUpdate();
                 ...
                 sql = "INSERT INTO nsaveprev (ind, zero, one, two, three, four, five, six, seven, eight) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                st = (PreparedStatement) dbconn.prepareStatement(sql);
            for (int i = 1; i < 10; i++) {
                for (int j = 0; j < number; j++) {
            ...
            sql = "INSERT INTO nsavefull (ind, zero, one, two, three, four, five, six, seven, eight) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                st = (PreparedStatement) dbconn.prepareStatement(sql);
            for (int i = 1; i < 10; i++) {
                for (int j = 0; j < number; j++) {
                    st.setString(j+2, Integer.toString(full[i-1][j])); 
             ...
             sql = "INSERT INTO nsavemat (ind, zero, one, two, three, four, five, six, seven, eight) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                st = (PreparedStatement) dbconn.prepareStatement(sql);
            for (int i = 1; i < 10; i++) {
                for (int j = 0; j < number; j++) {
            ...
            sql = "INSERT INTO nsaver (name, time, level, hint, chek) VALUES (?, ?, ?, ?, ?)";
                st = (PreparedStatement) dbconn.prepareStatement(sql);
